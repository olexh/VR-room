...
// To give energy on output, input need be connected to energized switchable components
// Like a switch
...
{
    public class ElectricGateNOR : ElectricComponent
    {
        ...
        public override void ActivateOutput()
        {
            ...
            for (int i = 0; i < wireOutput.Length; i++)
            {
                if (wireOutput[i] != null)
                {
                    ...
                    if (GetSetIsEnergized && !inputAO && !inputBO)
                    {
