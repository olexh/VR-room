...
{
    public class ElectricGateXNOR : ElectricComponent
    {
        ...
        public override void ActivateOutput()
        {
            ...
            for (int i = 0; i < wireOutput.Length; i++)
            {
                if (wireOutput[i] != null)
                {
                    ...
                    if (GetSetIsEnergized && ((!inputAO && !inputBO) || (inputAO && inputBO)))
                    {
                        wireOutput[i].GetComponent<WireControl>().wireConnectorInput.EnergizeByWire(true);
                    }
                    // Skip this one to not have to add another else to deactivate in case of not energized and 2 false input
                    else// if (inputAO || inputBO)
                    {
