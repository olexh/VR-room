...
{
    public class ElectricGateOR : ElectricComponent
    {
            ...
            for (int i = 0; i < wireOutput.Length; i++)
            {
                if (wireOutput[i] != null)
                {
                    if (GetSetIsEnergized &&
                        ((wireInput[0] != null && wireInput[0].GetComponent<WireControl>().wireConnectorOutput.IsOn()) ||
                         (wireInput[1] != null && wireInput[1].GetComponent<WireControl>().wireConnectorOutput.IsOn())))
                    {
